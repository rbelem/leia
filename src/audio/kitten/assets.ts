...
/** The 8 bundled nano voices, in the demo's order; Bella is the default. */
export const KITTEN_VOICE_NAMES = ["Bella", "Jasper", "Luna", "Bruno", "Rosie", "Hugo", "Kiki", "Leo"] as const;
...
/** voices.json shape: { "<name>": number[512], ... } (512-dim style embedding). */
export type KittenVoicesJson = Record<string, number[]>;

/**
 * Map a picker voice name onto its embedding, case-insensitively with a
 * trim/underscore-relaxed match (keys are plain names in the demo file, but
 * the exact casing is not contractual).
 */
export function resolveVoiceEmbedding(voices: KittenVoicesJson, name: string): Float32Array | null {
  const direct = voices[name];
  if (Array.isArray(direct)) return Float32Array.from(direct);
  const norm = (s: string): string => s.toLowerCase().replace(/[\s_-]/g, "");
  const wanted = norm(name);
  for (const [key, value] of Object.entries(voices)) {
    if (norm(key) === wanted && Array.isArray(value)) return Float32Array.from(value);
  }
  return null;
}
